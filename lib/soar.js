...
exports.config = function(options)  {
	...
	if (!options.dbConfig)
		options.dbConfig = JSON.parse( fs.readFileSync('../config.json') );
...
function readDVML(vfile, q, range, handler, op, conn)  {
	//var  fpath = basePath + vfile;
	var  fpath;
	if (path.sep === '/')
		fpath = vfile.charAt(0) === '/'  ?  vfile : basePath + vfile;
	else
		fpath = vfile.indexOf(':') > 0  ?  vfile : basePath + vfile;

	try  {
		var  xmlData = fs.readFileSync( fpath ),
			 parser = new xml2js.Parser();

		//parser.parseString( xmlData, {explicitArray: true }, function(err, result) {
		parser.parseString( xmlData, function(err, result) {
			if (err == null)  {
				...
				if (!(eDef instanceof Error))  {
					...
					if (op === 'q')
						querying( eDef, q, handler, conn );
					else  if (op === 'l')
						listing( eDef, q, range, handler, conn );
					else  if (op == 'u')
		...
		handler( new Error('Unable to read the definition file.') );
	}
};
...
exports.query = function query(vfile, q, handler, conn)  {
	...
	if (eDef === undefined)
		readDVML( vfile, q, null, handler, 'q', conn );
	else
		querying( eDef, q, handler, conn );
};
...
exports.list = function list(vfile, q, range, handler, conn)  {
	...
	var  eDef = defCache[vfile];
	if (eDef === undefined)
		readDVML( vfile, q, range, handler, 'l', conn );
	else
		listing( eDef, q, range, handler, conn );
};
...
function  querying(eDef, q, handler, conn)  {
	// 1. compose sql from eDef
	var  p = new Array(),
		 sql = mysql.composeQuery( eDef, q, p );
...
function  listing(eDef, q, range, handler, conn)  {
	// 1. compose sql from eDef
	var  p = new Array(),
		 sql = mysql.composeList( eDef, q, p, range );
