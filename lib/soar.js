	...
	if (conn)
		...
		exports.getConnection(function(err, conn)  {
			if (err)
				...
				exports.getSchemaManager().createTable(conn, schema, function(err, result) {
					conn.release();
					cb(err, result);
				});
		});
	...
	if (conn)
		...
		exports.getConnection(function(err, conn)  {
			if (err)
				...
				exports.getSchemaManager().alterTable(conn, schema, function(err) {
					conn.release();
					cb(err);
				});
		});
	...
	if (conn)
		...
		exports.getConnection(function(err, conn)  {
			if (err)
				...
				exports.getSchemaManager().deleteTable(conn, tbName, function(err) {
					conn.release();
					cb(err);
				});
		});
	...
	if (conn)
		...
		exports.getConnection(function(err, conn)  {
			if (err)
				...
				exports.getSchemaManager().describeTable(conn, tbName, function(err, schema) {
					conn.release();
					cb(err, schema);
				});
		});
...
function  autoFillSchema(dbName, tbName, options, data, query, handler)  {
	getTableSchema(dbName, tbName, function(err, schema) {
		...
		var  //stemp = exports.sqlTemplate(schema.title),
			 stemp = exports.sqlTemplate(tbName),	// how about 'table as alias'
