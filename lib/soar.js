...
var  basePath = {},
	 defCache = {},
	 schemaCache = {},	// cache the primary keys of each table
 ...
 */
exports.execute = function(options, handler)  {
	...
	// check options.expr.columns
	if (options.op !== 'delete' && !options.expr.columns)  {
		// we shall find out the columns of a table
		getTableSchema(dbName, tableLoc.tbName, function(err, schema) {
			options.expr.columns = Object.keys(schema.columns);
			runTemplate(tableLoc, options, handler);
		});
	}
	else
		runTemplate(tableLoc, options, handler);
...
function  runTemplate(tableLoc, options, handler)  {
	var  p = [],
		 sql = getJsonSqlGenerator().toSQL(options, p);
			...
			options.conn.query(sql, p, function(err, value) {
				if (err)
					...
					if (options.op === 'insert')
						returnInsert(tableLoc, options.data, value, handler);
					else  {
						if (options.op === 'query')
							value = value[0];
						handler(null, value);
